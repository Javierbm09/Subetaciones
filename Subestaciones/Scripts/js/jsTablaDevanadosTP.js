...
var _columnsTablaDevanados = [
    ...
    { text: 'CLase precisión', columntype: 'textbox', filtertype: 'input', dataField: 'ClasePresicion', align: 'left', width: '20%' },
    { text: 'Capacidad (VA)', columntype: 'textbox', filtertype: 'input', dataField: 'Capacidad', align: 'left', width: '15%' },
    {
        ...
        buttonclick: function (row) {
            ...
            editrow = row;
            var offset = $("#TablaDevanado").offset();
            $("#popEditaDevanados").jqxWindow({ position: { x: parseInt(offset.left) + 60, y: parseInt(offset.top) + 60 } });

            // get the clicked row's data and initialize the input fields.
            var datos = $("#TablaDevanado").jqxGrid('getrowdata', editrow);
            $("#_Nro_Dev").val(datos.Nro_Dev);
            $("#_Nro_Serie").val(datos.Nro_TP);
            $("#_tension").val(datos.tension);
            $("#_capacidad").val(datos.Capacidad);
            $("#_clase").val(datos.Clase_Precision);
            $("#_Designacion").val(datos.Designacion)

            // show the popup window.
            $("#popEditaDevanados").jqxWindow('open');
        }
    }
];
